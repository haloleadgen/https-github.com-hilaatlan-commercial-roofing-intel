---
term: "Ponding water"
title: "Ponding water"
description: "Water that remains on a roof surface more than 48 hours after rainfall ends — a leading accelerant of membrane aging and a common warranty exclusion."
alsoKnownAs: ["Standing water"]
status: sample
updated: "2026-07-01"
related:
  - title: "TPO Roofing Systems: A Complete Owner's Reference"
    href: "/knowledge/tpo-roofing-systems/"
    type: "Encyclopedia"
  - title: "Positive drainage (definition)"
    href: "/glossary/positive-drainage/"
    type: "Glossary"
---

## Plain-English definition

If water is still sitting on your roof two days after it stopped raining, that's ponding
water. Roofs are designed to shed water; where it collects instead, the membrane ages
faster, debris accumulates, and structural load increases.

## Why it matters to you

**Verified:** NRCA defines ponding as water remaining more than 48 hours after
rainfall under drying conditions, and manufacturer warranties commonly require positive
drainage or exclude damage from standing water
([NRCA guidance](https://hinarratives.com/wp-content/uploads/sites/176/2021/09/Roof-Ponding-NRCA.pdf),
[ARMA — Ponding Water Basics](https://www.asphaltroofing.org/ponding-water-basics/)).
Remediation typically means drainage correction: added drains, scuppers, or tapered
insulation.

## Technical detail

Placeholder: drainage-design causes (deck deflection, blocked drains, inadequate slope),
measurement conventions, and code references.
