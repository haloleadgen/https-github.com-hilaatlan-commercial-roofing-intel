---
title: "Repair or Replace? How to Decide"
description: "A structured decision framework for the most consequential commercial roofing decision — with cost thresholds, condition criteria, and the questions to ask before committing either way."
type: guide
status: published
updated: "2026-07-01"
sources: 6
takeaways:
  - "The decision is rarely about the leak in front of you — it's about the remaining value of the whole assembly."
  - "A common consultant rule of thumb: when repair costs approach 25–30% of replacement cost, replacement deserves serious analysis."
  - "Wet insulation changes the math: around 25% saturation, consultants typically find tear-off more economical than chasing moisture — and code prohibits recovering a water-soaked roof."
  - "Get the condition assessment before the proposal, not from the proposal."
related:
  - title: "Roof Life Expectancy Estimator"
    href: "/tools/roof-life-expectancy-estimator/"
    type: "Decision Tool"
  - title: "My Roof Is Leaking — What Do I Do Right Now?"
    href: "/situations/my-roof-is-leaking/"
    type: "Situation"
  - title: "TPO Roofing Systems: A Complete Owner's Reference"
    href: "/knowledge/tpo-roofing-systems/"
    type: "Encyclopedia"
---

## Step 1 — Establish the facts before anyone quotes you

Commission the evidence before soliciting proposals: an independent
[condition assessment](/knowledge/commercial-roof-inspections/), a moisture survey
(infrared or capacitance) to map wet insulation, core samples where the survey is
ambiguous, and a review of remaining [warranty](/knowledge/commercial-roof-warranties/)
coverage. A proposal written before this evidence exists is a guess with a price on it.

## Step 2 — Run the four decision tests

1. **Condition test.** How much of the assembly (membrane, insulation, deck) is actually compromised?
2. **Economic test.** Compare repair cost + remaining life against replacement cost + new life.
3. **Risk test.** What does failure cost you (tenants, inventory, operations, insurance position)?
4. **Timing test.** Can the decision be staged, or does deferral compound the damage?

## Step 3 — Interpret the results

| Evidence picture | Typical path |
| --- | --- |
| Dry assembly, localized defects | Targeted repair + proactive maintenance |
| Dry but UV-tired field, sound adhesion | [Restoration coating](/knowledge/compare/coating-vs-replacement/) |
| Saturation approaching the ~25% rule of thumb | Replacement analysis begins in earnest |
| Widespread saturation, attachment or structural issues | Replacement (staged if evidence supports it) |

Two thresholds anchor the analysis. Roof consultants commonly treat wet insulation
around 25% of roof area as the point where tear-off becomes more economical than
chasing moisture. Separately, building codes (IBC reroofing provisions) prohibit
recovering a water-soaked roof and trigger full code compliance when work exceeds
prescribed thresholds — meaning a "big repair" can legally become a replacement-grade
project ([NRCA, Reroofing and Code Compliance](https://nrcawebstorage.blob.core.windows.net/files/filesnrca/pdf/technical/Reroofing-Code-Compliance-Nov-2016.pdf)).

## When the answer is genuinely "it depends"

Boundary cases are real: a 60%-sound roof with a failing section, a dry roof at
code-minimum insulation, a saturated roof on a building being sold next year. In these
cases the honest answer is condition-dependent, and the professional assessments —
moisture survey, structural review, consultant's repair-vs-replace analysis — are worth
their fee precisely because the downside of guessing is measured in six figures. Per our
[Methodology](/about/methodology/), CRI states uncertainty rather than forcing false
confidence — and this decision has genuine uncertainty at its boundaries.
